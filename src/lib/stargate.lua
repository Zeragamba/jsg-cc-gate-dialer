...
function Stargate.connect()
    ...
    if not _stargate then
        ...
        print(
                "Check that both wired modems have a red circle on them. Right click to connect."
        )
        return false
    end
    ...
    function gate.resolveAddress(remoteAddress)
        ...
        local success_or_error, numGlyphs = _stargate.getSymbolsNeeded(remoteAddress)
